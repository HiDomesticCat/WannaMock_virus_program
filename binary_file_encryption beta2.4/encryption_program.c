...
void file_search(char *MainPosition){
  if(frequency == 0){
    ...
    char file[100000] = "cd "; //收集子資料夾的檔案用
    ...
    char file2[10000]; //幫助processed_original_file_name_read和processed_original_file_name_read讀取檔案
...
void file_read(char *MainPosition){
  ...
  char processed_original_file_name[10000]; //儲存除理過的原始檔名
  if(frequency == 0){
    ...
    while(fgets(processed_original_file_name, sizeof(processed_original_file_name), processed_original_file_name_read) != NULL){
      char encryption_file_name[10000] = ""; //儲存加密的檔名
    ...
    char file[100000]; //幫助processed_original_file_name_read讀取檔案
    ...
    char excessive_string[100000]; //中間轉換用
...
void folder_search(char *MainPosition){
  strtok(MainPosition, "\n");
  char folder[100000] = "cd "; //幫助尋找子資料夾
  ...
  char MainPosition_copy[100000]; //複製MainPosition
  strcpy(MainPosition_copy, MainPosition);
  char folder_name[100000]; //儲存子資料夾名稱
...
int main(void){
  char MainPosition[100000]; //儲存主資料夾位址
